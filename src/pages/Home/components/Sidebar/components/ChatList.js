...
const ChatList = () => {
    ...
    const showLastMessage = (chat) => {
        if (chat.messages.results.length>0){
            const message = chat.messages.results[chat.messages.results.length-1]
            if (message.image){
                return <img className='last-message-img'  src={message.image} alt='last received' />
            }
            if (message.file){
                if(message.file_details.file_name.length > 16){
                    return  <p><CustomFileIcon fileType={splitFileExtension(message.file_details.file_name)} color='red'/>{message.file_details.file_name.slice(0,16)}...</p>
                }
                return <p><CustomFileIcon color='red'/>{message.file_details.file_name}</p>
            }
            if(message.audio){
                return <LiaFileAudio/>
            }
        ...
        <div className="chatlist-container">
            {
                chatList&&chatList.length>0?chatList.map((chat) => (
                    <div key={chat.id} id={`chat-container-${chat.id}`}  
                        ...
                        <div className="chat-details">
                            <h4>{nameCheck(chat)} {checkUserOnline(chat.participants)}</h4>
                            {showLastMessage(chat)}
                        </div>
                        <div className="date-time">
                            {unReadMsgCount(chat.unread)}                                
                            <div>{timeDateConvertor(chat.messages.results)}</div>
                        </div>                      
                    </div>
                    )):
                    chatChecked?
                    <p style={{padding:'20px 30px',textAlign:'center',fontSize:'22px'}}>Search for users and send message <FaArrowUp /> </p>
                    :<Loader/>
            }
        </div>
    )

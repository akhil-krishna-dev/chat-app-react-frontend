...
const ProfileSettings = () => {
    const {authUser,updateUserFullName,updateUserStatus,fetchUser} = useContext(UserContext)
    ...
    const handleSavingProfileImage = async () => {
        ...
        try {
            const response = await axios.post(`${BaseUrl}accounts/users/change-profile-image`, formData, {
                headers: {
   ...
   const editUserName = async () => {
        ...
        const { value: formValues } = await Swal.fire({
          ...
          if (formValues) {
            ...
            .post(`${BaseUrl}accounts/users/edit-username`,
                ...
                updateUserFullName(response.data.full_name)
                Swal.fire({
                    text:"Your name changed to "+formValues[0]+" "+formValues[1],
                    icon:"success"
                })
            }).catch((error) => {
                console.log(error)
    ...
    const renderImageChangeAction = () => {
        ...
        if (croppedImage){
            return  <IoIosCheckmarkCircle onClick={handleSavingProfileImage} className="image-save-btn" size={50} />
        }
    }
        ...
        <>
            {
                ...
                <div className="profile-settings-container">
                    <div className="profile-image-container">
...

                            {
                               ...
                               (
                                    ...
                                    >
                                        <img onLoad={onImageLoaded} src={selectedImage} alt="cropped-profile.jpg" />
                                    </ReactCrop>
                               ):
                               (
                                    <div onClick={triggerFileInput} className="image">
                                        <img src={"http://127.0.0.1:8000"+authUser.image} alt="profile.jpg" />
                                    </div>
                               )
                            }
                    </div>
                    ...
                    <input onChange={onSelectFile} style={{display:"none"}} ref={inputRef} type="file" accept="image/*" />
                    <div className="settings-container">
                        <div  className="name">
                            <h4>Your name</h4>
                            <span>{authUser.full_name}</span> <GrEdit onClick={editUserName} className="username-edit-btn" size={20}/> 
                        </div>
                        <div className="status">
                            <h4>status</h4>
                            <span> {authUser.status}</span> <GrEdit onClick={editUserStatus} className="username-edit-btn" size={20}/>
                        </div>
                    </div>
                    <Logout/>
                </div>
